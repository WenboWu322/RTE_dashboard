...
def create_model(P, T, Pinit, Rcr, Rre, r_cr, r_re, P_rempli, alpha,alpha_param,deg, taux1, taux2, gamma, M, couts1, couts2, b1, b2, k1, k2, k3, W, K):
    ...
    for i in range(P):
        # leakage_expr = mdl.sum(K * v[i, t] for t in range(1,T))
        leakage_expr = mdl.sum(K * v[i, tp] for tp in range(max(0, t - W + 1), t + 1))
        leakageQuantity[i] = leakage_expr
        mdl.add_constraint(leakageQuantity[i] <= 0.42)

        
    # leakageQuantity = mdl.sum(K * (p[i, t] - p[i, t+1]) for i in range(P) for t in range(T-1))
    # mdl.add_constraint(leakageQuantity <= 2.5)
    ...
    # total_leakage_value = round(sum(K * v[i, t] * (si[p[i, t - 1]] - si[p[i, t]]) for i in range(P) for t in range(1, T)), 3)
    # total_leakage_value_epr = sum(K * (si[p[i, t - 1]] - si[p[i, t]]) for i in range(P) for t in range(1, T))
    total_leakage_value_epr = sum(si[leakageQuantity[i]] for i in range(P))
    total_leakage_value = round(total_leakage_value_epr, 3)

    # 打印每个泄漏量的值
    for i in range(P):
        st.write(f"Leakage quantity for unit {i}: {si[leakageQuantity[i]]}")

    # 如果你需要总泄漏量
    total_leakage_value = sum(si[leakageQuantity[i]] for i in range(P))
    st.write(f"Total leakage quantity: {total_leakage_value}")
